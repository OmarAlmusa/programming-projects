...
class OfInterface(customtkinter.CTkToplevel):
    def __init__(self, img_size):
        ...
        self.button1 = ButtonFrame(self, "Sparse Optical Flow", "images/sparse_optical_flow_template.jpg",
                                   self.img_size,
                                    0.6,
                                    command = lambda : self.open_sparse_interface(self.img_size))

        self.button2 = ButtonFrame(self, "Dense Optical Flow", "images/dense_optical_flow_template.jpg",
                                   self.img_size,
                                   0.6,
                                   command = lambda : self.open_dense_interface(self.img_size))
    ...
    def open_sparse_interface(self, img_size):
        window = sof_interface.Video_interface(img_size)
        window.title("sparse optical flow")

    def open_dense_interface(self, img_size):
        window = dof_interface.Video_interface(img_size)
