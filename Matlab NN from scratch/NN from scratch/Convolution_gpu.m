classdef Convolution_gpu < handle
    ...
    methods
        function obj = Convolution_gpu(input_shape, kernel_size, depth, initialization_type)
            ...
            obj.dw = gpuArray(zeros(obj.kernel_shape));
            obj.db = gpuArray(zeros(obj.output_shape));
                
            if type == 'NX'
                obj.weights = gpuArray(Normalized_Xavier(in_s, n_size, obj.kernel_shape));
                obj.bias = gpuArray(Normalized_Xavier(in_s, n_size, obj.output_shape));
                
            elseif type == 'HE'
                obj.weights = gpuArray(He_Weight(in_s, obj.kernel_shape));
                obj.bias = gpuArray(He_Weight(in_s, obj.output_shape));
                
            else
                obj.weights = gpuArray(Xavier(in_s, obj.kernel_shape));
                obj.bias = gpuArray(Xavier(in_s, obj.output_shape));
            end
        ...
        function output = backward(obj, grad, learning_rate)
            obj.dx = gpuArray(zeros(obj.input_shape));
