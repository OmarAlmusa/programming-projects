classdef Dense_gpu < handle
    ...
    methods
        function obj = Dense_gpu(in_dim, n_neurons, initialization_type)
            type = initialization_type;
            obj.n_neurons = n_neurons;
            obj.in_dim = in_dim;
            obj.dw = gpuArray(zeros(obj.n_neurons, obj.in_dim));
            obj.db = gpuArray(zeros(obj.n_neurons, 1));
            
            if type == 'NX'
                obj.weights = gpuArray(Normalized_Xavier(in_dim, n_neurons, [n_neurons, in_dim]));
                obj.bias = gpuArray(Normalized_Xavier(in_dim, n_neurons, [n_neurons, 1]));
            
            elseif type == 'HE'
                obj.weights = gpuArray(He_Weight(in_dim, [n_neurons, in_dim]));
                obj.bias = gpuArray(He_Weight(in_dim, [n_neurons, 1]));
                
            else
                obj.weights = gpuArray(Xavier(in_dim, [n_neurons, in_dim]));
                obj.bias = gpuArray(Xavier(in_dim, [n_neurons, 1]));
            end
